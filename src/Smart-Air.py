...
# Data is in ../data/processed/ relative to src/
...
def get_data(data_name):
    timestamp = pd.date_range(
        start='2022-04-17 00:40:00',
        end="2022-06-01 00:00:00",
        freq='10T'
    )
    
    # Fixed path for Streamlit Cloud
    ...
    data = pd.DataFrame({
        ...
        'Relative Humidity (%)': data.iloc[:, 1][144:],
    ...
    data["Pressure Controller Condition"] = ["Stable" for i in range(len(data))]

    for row in range(len(data)):
        if data['Temperature (°C)'].iloc[row] <= 20:
            data["AC Condition"].iloc[row] = "AC mode in <20°C"
        elif data['Temperature (°C)'].iloc[row] >= 26:
            data["AC Condition"].iloc[row] = "AC mode in >26°C"
        elif data['Relative Humidity (%)'].iloc[row] <= 25:
            data["Humidifier Condition"].iloc[row] = "Humidifier mode in 25%"
        elif data['Relative Humidity (%)'].iloc[row] >= 45:
            data["Humidifier Condition"].iloc[row] = "Humidifier mode in 45%"
        elif data["Air Pressure (Pa)"].iloc[row] <= 99000:
            data["Pressure Controller Condition"].iloc[row] = "Pressure Contr. mode in < 99k"
        elif data["Air Pressure (Pa)"].iloc[row] >= 101000:
            data["Pressure Controller Condition"].iloc[row] = "Pressure Contr. mode in > 101k"
    return data
...
if selected_room is not None:
    for key, value in rooms_dict.items():
        if selected_room == key:
            data = get_data(value)
    options = ["Temperature (°C)", "Relative Humidity (%)", "Air Pressure (Pa)", None]
...
if selected_item is not None:
    ...
    selected_day = st.date_input(
        "Select a date:",
        value=datetime.date(2022, 4, 17),
        min_value=datetime.date(2022, 4, 17),
        max_value=datetime.date(2022, 5, 29)
    )
    ...
    if selected_item == 'Relative Humidity (%)':
        st.write(data[['Relative Humidity (%)', "Humidifier Condition"]][index_start:index_end])
        cases = [False, True, False]
    if selected_item == "Air Pressure (Pa)":
        st.write(data[["Air Pressure (Pa)", "Pressure Controller Condition"]][index_start:index_end])
        cases = [False, False, True]

    choice = st.selectbox(
        "Select your choice:",
        ["Get Stable", "Schedule", None],
        index=2
    )
    ...
    selected_datetime = datetime.datetime.combine(selected_datetime, datetime.datetime.min.time())
    ...
    if cases[0] and choice == "Get Stable" and interval_size is not None:
        data["AC Condition"].iloc[start_time:end_time] = 'Stabled'
        st.write("Manipulated Data:", data[['Temperature (°C)', "AC Condition"]][index_start:index_end])
        button_function()
    if cases[1] and choice == "Get Stable" and interval_size is not None:
        data["Humidifier Condition"].iloc[start_time:end_time] = 'Stabled'
        st.write("Manipulated Data:", data[['Relative Humidity (%)', "Humidifier Condition"]][index_start:index_end])
        button_function()
    if cases[2] and choice == "Get Stable" and interval_size is not None:
        data["Pressure Controller Condition"].iloc[start_time:end_time] = 'Stabled'
        st.write("Manipulated Data:", data[["Air Pressure (Pa)", "Pressure Controller Condition"]][index_start:index_end])
        button_function()
    if cases[0] and choice == "Schedule" and interval_size is not None:
        data["AC Condition"].iloc[start_time:end_time] = 'Scheduled'
        st.write("Manipulated Data:", data[['Temperature (°C)', "AC Condition"]][index_start:index_end])
        button_function()
    if cases[1] and choice == "Schedule" and interval_size is not None:
        data["Humidifier Condition"].iloc[start_time:end_time] = 'Scheduled'
        st.write("Manipulated Data:", data[['Relative Humidity (%)', "Humidifier Condition"]][index_start:index_end])
        button_function()
    if cases[2] and choice == "Schedule" and interval_size is not None:
        data["Pressure Controller Condition"].iloc[start_time:end_time] = 'Scheduled'
        st.write("Manipulated Data:", data[["Air Pressure (Pa)", "Pressure Controller Condition"]][index_start:index_end])
        button_function()